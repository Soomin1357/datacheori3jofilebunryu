...
class DocumentExtractor:
    ...
    def _light_ocr_pdf(self, file_path):
        if not HAS_OCR: return self._fallback_filename(os.path.basename(file_path), reason="OCR 미설치 (스캔 PDF)")
        try:
            doc = fitz.open(file_path)
            page = doc[0]
            rect = page.rect
            ...
            with col_btn1:
                if st.button("📦 최종 설정 반영하여 ZIP 파일 생성하기", use_container_width=True, type="secondary"):
                    with st.spinner("지정한 폴더 계층 구조와 커스텀 설명에 맞추어 ZIP 구조 빌드 중..."):
                        ...
                        # 💡 파일 헤더 충돌을 피하기 위해 내부 아카이브 구조를 안전한 영문으로 매핑
                        # 대신 최상위에 설명서 가이드 메모장(.txt)을 동적으로 생성해 제공
